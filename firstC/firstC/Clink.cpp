...
//从链表中删除某个学生
void Clink::DeleteStu(CStudent * pHeader, char * pName){
...
//将某个学生插入到链表中
void Clink::InsertStu(CStudent * pHeader, CStudent* aNewStu){
...
//从根据名字从链表中寻找某个学生
void Clink::FindStudentByName(CStudent*pHeader, char * pName){
	CStudent*pTemp = pHeader->Next;
	while (pTemp != 0){
		if (strcmp(pTemp->Name, pName) == 0){
...
//从根据ID从链表中寻找某个学生
void Clink::FindStudentByID(CStudent*pHeader, char * id){
	CStudent*pTemp = pHeader->Next;
	while (pTemp != 0){
...
//求平均成绩
double Clink::GetAverageStore(CStudent * pHeader){
	double average;
...
//根据名字排序
void Clink::SortByName(CStudent * pHeader, bool bAscend){
	int m = 0;
	CStudent*pTemp = pHeader->Next;
...
//根据ID排序
void Clink::SortByID(CStudent * pHeader, bool bAscend){
	int m = 0;
	CStudent*pTemp = pHeader->Next;
	CStudent*pBtemp = pHeader;
	CStudent*pNext_temp = pTemp->Next;
...
//根据成绩排序
void Clink::SortByScore(CStudent * pHeader, EnumScoreType enumScoreType, bool bAscend){
	int m = 0;
	CStudent*pTemp = pHeader->Next;
	CStudent*pBtemp = pHeader;
	CStudent*pNext_temp = pTemp->Next;
...
void Clink::InitializeStruct(CStudent * pHeader)
{
...
void Clink::OutPutStudent(CStudent * pHeader){
	CStudent * pTemp = pHeader->Next;
	while (0 != pTemp)
	{
...
void Clink::output_a_student(CStudent*pHeader){
	cout << pHeader->ID << "  " << pHeader->Name << "  ";
	for (int i = 0; i <3; i++){
		cout << pHeader->nScore[i] << "  ";
	}
}
struct CStudent* Clink::name_a_student(){
	char name[10];
	char id[20];
	int score[3];
	cout << "输入名字：" << endl;
